...
def test_bad_settings(db_test_app):
    ...
    with pytest.raises(ValidationError):
        results, node = run_get_node(
            ...
            settings=DataFactory("dict")(dict={"a": 1}),
        )
...
def test_no_structure(db_test_app):
    ...
    wflow_cls = WorkflowFactory("crystal17.sym3d")
    results, node = run_get_node(
        wflow_cls, settings=DataFactory("dict")(dict={"symprec": 0.01})
    )
...
def test_with_structure(db_test_app, get_structure, data_regression):
    ...
    results, node = run_get_node(
        WorkflowFactory("crystal17.sym3d"),
        settings=DataFactory("dict")(dict={"symprec": 0.01}),
...
def test_with_cif(db_test_app, get_cif, data_regression):
    ...
    results, node = run_get_node(
        WorkflowFactory("crystal17.sym3d"),
        settings=DataFactory("dict")(dict={"cif_parser": "ase", "symprec": 0.01}),
        cif=get_cif("pyrite"),
    )
    assert node.is_finished_ok, node.exit_status
    assert "symmetry" in results
...
@pytest.mark.parametrize(
    "compute_primitive,standardize_cell", [(True, False), (False, True), (True, True)]
)
def test_symmetrise_structure(
    db_test_app, get_structure, compute_primitive, standardize_cell, data_regression
):
    """symmetrising structure with different options """
    results, node = run_get_node(
        WorkflowFactory("crystal17.sym3d"),
        settings=DataFactory("dict")(
            dict={
                "symprec": 0.01,
...
def test_new_kind_names_fail(db_test_app, get_structure):
    ...
    results, node = run_get_node(
        wflow_cls,
        settings=DataFactory("dict")(dict={"symprec": 0.01, "kind_names": ["A"]}),
        structure=get_structure("zincblende"),
    )
