...
def get_cx1_crystal_code(work_dir, exec_path, modules=('intel-suite/2016.3', 'mpi/intel-5.1'), key_filename=None):
...
def submit_nio_afm_fullopt():
    ...
    code = get_cx1_crystal_code('/work/cjs14/aiida_v100b3_runs',
    ...
    sym_settings = DataFactory('dict')(dict={'symprec': 0.01, 'compute_primitive': True})

    sym_calc = run_get_node(WorkflowFactory('crystal17.sym3d'), structure=ostruct, settings=sym_settings).node
    time.sleep(0.1)
    ...
    group, created = Group.objects.get_or_create(label='cry17_cx1_tests')  # type: Group
    group.add_nodes([code, ostruct, kind_data, sym_settings, sym_calc, instruct, symmetry, calc_node])
