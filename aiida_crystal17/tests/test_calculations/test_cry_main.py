...
def test_create_builder(db_test_app, get_structure):
    ...
    sym_calc = run_get_node(
        ...
        structure=instruct,
        settings=DataFactory("dict")(dict={"symprec": 0.01, "compute_primitive": True}),
    ).node
...
def test_calcjob_submit_mgo(
    db_test_app, input_symmetry, get_structure, data_regression, file_regression
):
    """Test submitting a calculation."""
    code = db_test_app.get_or_create_code("crystal17.main")

    # Prepare input parameters
    inparams = CryInputParamsData(
        data={"title": "MgO Bulk", "scf": {"k_points": (8, 8)}}
    )
    ...
    sym_calc = run_get_node(
        WorkflowFactory("crystal17.sym3d"),
        structure=instruct,
        settings=orm.Dict(dict={"symprec": 0.01, "compute_primitive": True}),
    ).node
    instruct = sym_calc.get_outgoing().get_node_by_label("structure")
    symmetry = sym_calc.get_outgoing().get_node_by_label("symmetry")
...
def test_calcjob_submit_nio_afm(
    db_test_app,
    get_structure,
...
def test_restart_wf_submit(
    ...
    with resource_context("crystal", "nio_sto3g_afm_scf_maxcyc") as path:
        builder.wf_folder = orm.RemoteData(
            computer=code.computer, remote_path=str(path)
        )
...
@pytest.mark.cry17_calls_executable(
    skip_non_mock=True, reason="the calculation takes about 50 minutes to run"
)
