...
@pytest.mark.parametrize(
    "plugin_name,fcontent,error_msg",
    [
        (
            "crystal17.main",
            "=>> PBS: job killed: mem job total 1 kb exceeded limit 10 kb",
            "ERROR_OUT_OF_MEMORY",
        ),
        (
            "crystal17.main",
            "=>> PBS: job killed: vmem job total 1 kb exceeded limit 10 kb",
            "ERROR_OUT_OF_VMEMORY",
        ),
        (
            "crystal17.main",
            "=>> PBS: job killed: walltime 100 exceeded limit 10",
...
def test_failed_geom_convergence(db_test_app, plugin_name):
    ...
    with open_resource_binary(
        "crystal", "failed", "FAILED_GEOM_mackinawite_opt.out"
    ) as handle:
        retrieved.put_object_from_filelike(handle, "main.out", mode="wb")
    ...
    assert (
        calcfunction.exit_status
        == calc_node.process_class.exit_codes.UNCONVERGED_GEOMETRY.status
    )
