...
def standard_structure(structure, settings):
    ...
    return standardize_cell(
        structure, settings["symprec"], atol,
...
class Symmetrise3DStructure(WorkChain):
    ...
    def define(cls, spec):
        ...
        spec.input("settings", valid_type=DataFactory("dict"),
                   serializer=to_aiida_type,
                   required=True, validator=cls.validate_settings)
        ...
        spec.exit_code(302, 'ERROR_COMPUTE_OPTIONS',
                       message='idealize can only be used when standardize=True')
        spec.exit_code(303, 'ERROR_RESET_KIND_NAMES',
                       message='the kind names supplied are not compatible with the structure')
        spec.exit_code(304, 'ERROR_NEW_STRUCTURE',
                       message='error creating new structure')
        spec.exit_code(305, 'ERROR_COMPUTING_SYMMETRY',
