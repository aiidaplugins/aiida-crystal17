...
def test_bad_settings(db_test_app):
    ...
    with pytest.raises(ValidationError):
        results, node = run_get_node(
            WorkflowFactory("crystal17.sym3d"), settings={"a": 1})


def test_no_structure(db_test_app):
    """test no StructureData or CifData """
    wflow_cls = WorkflowFactory("crystal17.sym3d")
    results, node = run_get_node(wflow_cls, settings={"symprec": 0.01})
...
def test_with_structure(db_test_app, get_structure, data_regression):
    ...
    results, node = run_get_node(
        WorkflowFactory("crystal17.sym3d"), settings={"symprec": 0.01},
        structure=get_structure("pyrite"))

    assert node.is_finished_ok, node.exit_status
    assert "symmetry" in results
    assert "structure" not in results
    data_regression.check(results["symmetry"].attributes)
...
@pytest.mark.parametrize('compute_primitive,standardize_cell', [
    (True, False),
    (False, True),
    (True, True)
])
def test_symmetrise_structure(db_test_app, get_structure,
                              compute_primitive, standardize_cell, data_regression):
    """symmetrising structure with different options """
    results, node = run_get_node(
        WorkflowFactory("crystal17.sym3d"), settings={
            "symprec": 0.01,
...
@pytest.mark.parametrize('compute_primitive', [
    True,
    False
])
