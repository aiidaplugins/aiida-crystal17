...
class CryMainBaseWorkChain(BaseRestartWorkChain):
    ...
    def setup(self):
        """Call the `setup` of the `BaseRestartWorkChain`,
        and then create the inputs dictionary in `self.ctx.inputs`.

        This `self.ctx.inputs` dictionary will be used by the `BaseRestartWorkChain`
        to submit the calculations in the internal loop.
        """
    ...
    def validate_parameters(self):
        """Validate inputs that might depend on each other
        and cannot be validated by the spec.

        Also define dictionary `inputs` in the context,
        that will contain the inputs for the calculation that will be
        ...
        if "kpoints_distance" in self.inputs:
            ...
            if curr_kpoints != [is_value, isp_value]:
                self.report(
                    "changing the intput kpoints ({0}) to the computed kpoints ({1})".format(
    ...
    def validate_basis_sets(self):
        ...
        Either the basis sets should be defined explicitly in the `basissets` namespace,
        or alternatively, a family can be defined in `basis_family`
        that will be used together with the input `StructureData` to generate the
        required mapping.
        """
        ...
        try:
            self.ctx.inputs.basissets = BasisSetData.prepare_and_validate_inputs(
                structure, basissets, basis_family
            )
    ...
    def validate_resources(self):
        ...
        One can omit the normally required `options.resources`
        input for the `PwCalculation`, as long as the input
        `automatic_parallelization` is specified.
        If this is not the case, the `metadata.options` should at least
        contain the options `resources` and `max_wallclock_seconds`,
    ...
    def prepare_calculation(self):
        ...
        If a `restart_calc` has been set in the context,
        its `remote_folder` will be used as the `wf_folder` input
        ...
        if self.ctx.restart_calc:
            ...
            if "optimisation" in self.ctx.restart_calc.outputs:
                ...
                self.ctx.inputs.structure = (
                    self.ctx.restart_calc.outputs.optimisation.get_step_structure(
            ...
            if self.ctx.use_fort9_restart:
                ...
                # but should use the exponential-backoff / pause functionality,
                # to connect to the remote or, at least test for a crystal read failure,
                # and restart without it the failure looks like:
    ...
    def report_error_handled(self, calculation, action):
        ...
        This should be called in a registered error handler
        if its condition is met and an action was taken.
...
def _handle_electronic_convergence_not_achieved(self, calculation):
    ...
    if (
        ...
        fmixing = (
            ...
            .get("numerical", {})
            .get("FMIXING", self.defaults.fmixing)
        )
        fmixing_new = int(fmixing * factor)

        self.ctx.restart_calc = calculation
        self.ctx.use_fort9_restart = True
        self.ctx.inputs.parameters["scf"].setdefault("numerical", {})[
            "FMIXING"
        ] = fmixing_new

        action = (
            "reduced fmixing from {} to {} and restarting from last calculation".format(
                fmixing, fmixing_new
            )
        )
