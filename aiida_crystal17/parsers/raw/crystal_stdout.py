...
KNOWN_ERRORS = (
    ...
    ("UNIT CELL NOT NEUTRAL", "UNIT_CELL_NOT_NEUTRAL"),
    ("TOO MANY OPEN SHELLS OF THE SAME SYMMETRY", "SHELL_SYMMETRY_ERROR"),
...
def read_crystal_stdout(content):
    ...
    outcome = parse_section(
        parse_geometry_input, lines, lineno, output, "geometry_input"
    )
    if outcome is None or outcome.parser_error is not None:
        return assign_exit_code(output)
    lineno = outcome.next_lineno
    ...
    outcome = parse_section(
        parse_scf_section, lines, lineno, output, ("initial_scf", "cycles")
    )
    if outcome is None or outcome.parser_error is not None:
        return assign_exit_code(output)
    lineno = outcome.next_lineno
    ...
    outcome = parse_section(
        parse_scf_final_energy, lines, lineno, output, ("initial_scf", "final_energy")
    )
    ...
    if "mulliken" in start_lines:
        outcome = parse_section(
            parse_mulliken_analysis, lines, start_lines["mulliken"], output, "mulliken"
        )
        # Note: we don't abort on error
        if outcome is not None:
            lineno = outcome.next_lineno
...
def strip_non_program_output(content):
    ...
    regex = re.compile(
        "^\\s*PROCESS\\s*\\d+\\s*OF\\s*\\d+\\s*WORKING[^\\S\n\r]*(\r\n|\r|\n)",
        re.MULTILINE,
    )
    ...
    warnings = [li.strip() for li, _ in re.findall(regex, content)]
    content = re.sub(regex, "", content)
...
def initial_parse(lines):
    ...
    for lineno, line in enumerate(lines):
        ...
        # search for an optimisation
            ...
            "CONVERGENCE ON GRADIENTS SATISFIED AFTER THE FIRST OPTIMIZATION CYCLE"
            in line
        ):
            if "optimization" in start_lines:
                if second_opt_line:
...
def parse_pre_header(lines, initial_lineno=0):
    ...
    return ParsedSection(
        lineno, meta_data, "couldn't find start of program header (denoted *****)"
    )
...
def parse_calculation_header(lines, initial_lineno):
    ...
    for i, line in enumerate(lines[initial_lineno:]):
        ...
        if re.findall("public\\s\\:\\s(.+)\\s\\-", line):
            data["crystal_subversion"] = re.findall("public\\s\\:\\s(.+)\\s\\-", line)[
                0
            ]
...
def parse_geometry_input(lines, initial_lineno):
    ...
    return ParsedSection(
        lineno, data, "couldn't find end of geometry input (denoted * GEOMETRY EDITING)"
    )
...
def parse_calculation_setup(lines, initial_lineno):
    ...
    for i, line in enumerate(lines[initial_lineno:]):
        ...
        if line.startswith("CRYSTAL - SCF - TYPE OF CALCULATION :"):
            ...
            data["calculation"]["type"] = (
                line.replace("TYPE OF CALCULATION :", "").strip().lower()
            )
    ...
    if end_lineno is None:
        return ParsedSection(
            curr_lineno, data, "couldn't find start of initial scf calculation"
        )
...
def parse_geometry_section(data, initial_lineno, line, lines):
    ...
    for pattern, field, pattern2 in [
        ...
        if fnmatch(line, pattern):
            if not fnmatch(lines[initial_lineno + 1].strip(), "A*B*C*ALPHA*BETA*GAMMA"):
                raise IOError(
                    "was expecting A B C ALPHA BETA GAMMA on line:"
                    " {0}, got: {1}".format(
                        initial_lineno + 1, lines[initial_lineno + 1]
                    )
                )
            ...
            if not fnmatch(lines[initial_lineno + 1].strip(), "ATOM*X/A*Y/B*Z/C"):
                ...
                if fnmatch(
                    lines[initial_lineno + 1].strip(), "ATOM*X/A*Y/B*Z(ANGSTROM)*"
                ):
                    ...
                    data[field] = edict.merge(
                        [data.get(field, {}), {"cell_parameters": cell_params}]
                    )
                else:
                    raise IOError(
            ...
            while (
                ...
                if all(periodic):
                    ...
                    atom_data.setdefault("fcoords", []).append(
                        [float(fields[4]), float(fields[5]), float(fields[6]) / c]
                    )
    ...
    if fnmatch(line, "CARTESIAN COORDINATES - PRIMITIVE CELL*"):
        ...
        while (
            lines[curr_lineno].strip() and not lines[curr_lineno].strip()[0].isalpha()
        ):
            fields = lines[curr_lineno].strip().split()
            if len(fields) < 6:
                raise IOError(
                    "was expecting ID ANUM SYMBOL X Y Z on line:"
        ...
        if "ANGSTROM" not in line:
            raise IOError(
                "was expecting lattice vectors in angstroms on line:"
                " {0}, got: {1}".format(initial_lineno, line)
            )
        if not fnmatch(lines[initial_lineno + 1].strip(), "X*Y*Z"):
            raise IOError(
                "was expecting X Y Z on line:"
        ...
        if "cell_vectors" in data["crystallographic_cell"]:
            raise IOError(
                "found multiple cell vectors on line:"
                " {0}, got: {1}".format(initial_lineno + 1, lines[initial_lineno + 1])
            )
        vectors = {
    ...
    if fnmatch(line, "*SYMMOPS - TRANSLATORS IN FRACTIONAL UNITS*"):
        ...
        if not fnmatch(
            ...
            raise IOError(
                ...
                " {0}, got: {1}".format(
                    initial_lineno + 1, lines[initial_lineno + 1].strip()
                )
            )
        if not fnmatch(
            lines[initial_lineno + 2], "*V*INV*ROTATION MATRICES*TRANSLATORS*"
        ):
            raise IOError(
                "was expecting symmetry headers on this line"
                " {0}, got: {1}".format(
                    initial_lineno + 2, lines[initial_lineno + 2].strip()
                )
            )
        symmops = []
        for j in range(nsymmops):
            ...
            if not len(values) == 14:
                raise IOError(
                    ...
                    " {0}, got: {1}".format(
                        initial_lineno + 3 + j, lines[initial_lineno + 3 + j].strip()
                    )
                )
...
def parse_scf_section(lines, initial_lineno, final_lineno=None):
    ...
    for k, line in enumerate(lines[initial_lineno:]):
        ...
        if "SCF ENDED" in line or (
            final_lineno is not None and curr_lineno == final_lineno
        ):
            # add last scf cycle
            if scf_cyc:
                scf.append(scf_cyc)
        ...
        if fnmatch(line, "CYC*"):
            ...
            if last_cyc_num is not None:
                if cur_cyc_num != last_cyc_num + 1:
                    return ParsedSection(
                        curr_lineno,
                        scf,
                        "was expecting the SCF cyle number to be {0} in line {1}: {2}".format(
            ...
            if fnmatch(line, "*ETOT*"):
                if not fnmatch(line, "*ETOT(AU)*"):
                    raise IOError(
                        "was expecting units in a.u. on line {0}, "
        ...
        if line.startswith("CHARGE NORMALIZATION FACTOR"):
            scf_cyc[
                "CHARGE NORMALIZATION FACTOR".lower().replace(" ", "_")
            ] = split_numbers(line)[0]
    ...
    for i, line in enumerate(lines[initial_lineno:]):
        ...
        if fnmatch(line.strip(), "TOTAL ENERGY*DE*"):
            ...
            if "total_corrected" in scf_energy:
                raise IOError(
                    "total corrected energy found twice, on line:"
                    " {0}, got: {1}".format(initial_lineno + i, line)
                )
        ...
        for k, line in enumerate(lines[initial_lineno:]):
            ...
            if "OPT END -" in line:
                ...
                data = [
                    {
                        "energy": {
                            "total_corrected": convert_units(
                                split_numbers(line)[0], "hartree", "eV"
                            )
                        }
                    }
                ]
        ...
        if "CRYSTAL - SCF - TYPE OF CALCULATION :" in line:
            if scf_start_no is not None:
                return ParsedSection(
                    curr_lineno,
                    opt_cycles,
                    "found two lines starting scf ('CRYSTAL - SCF - ') in opt step {0}:".format(
                        len(opt_cycles)
                    )
        ...
        if "BAND GAP" in line:
            if fnmatch(line.strip(), "ALPHA BAND GAP:*eV"):
                ...
                return ParsedSection(
                    initial_lineno,
                    band_gaps,
                    "found a band gap of unknown format at line {0}: {1}".format(
                        curr_lineno, line
                    ),
                )
            if bgtype in band_gaps:
                return ParsedSection(
                    initial_lineno,
                    band_gaps,
                    "band gap data already contains {0} value before line {1}: {2}".format(
...
def parse_mulliken_analysis(lines, mulliken_indices):
    ...
    for i, indx in enumerate(mulliken_indices):
        ...
        if not (
            name == "ALPHA+BETA ELECTRONS".lower()
            ...
            if fnmatch(line.strip(), "*ATOM*Z*CHARGE*A.O.*POPULATION*"):
                ...
                while (
                    ...
                    if len(fields) != len(split_numbers(searchlines[charge_line])):
                        ...
                        data_shell.setdefault("atomic_numbers", []).append(
                            int(fields[2])
                        )
...
def extract_final_info(parsed_data):
    ...
    if "primitive_cell" not in data:
        if "optimisation" in parsed_data:
            ...
            data["primitive_cell"] = copy.deepcopy(
                parsed_data["initial_geometry"].get("primitive_cell", None)
            )
        else:
    ...
    if "primitive_symmops" not in data:
        if "optimisation" in parsed_data:
            raise ValueError(
                "optimisation, but no primitive_symops specified in final_geometry"
            )
        if (
            "initial_geometry" in parsed_data
            and "primitive_symmops" in parsed_data["initial_geometry"]
        ):
